...
gdc_image_id = os.listdir(gdc_data_path)
tcia_image_id = os.listdir(tcia_data_path)

#gdc_image_id = gdc_image_id[:int(0.2*len(gdc_image_id))] # 20% of the data as a test
#gtex_image_id = gtex_image_id[:int(0.2*len(gtex_image_id))] # 20% of the data as a test

paths = [gdc_data_path + "/" + case for case in gdc_image_id] + [tcia_data_path] #[gtex_data_path + "/" + case for case in gtex_image_id] # All case folders paths
formats = [".svs"]
case_ids = []
labels = []
...
# %%
for i in range(SPLITS):
    with open(f"C:\\Users\\Alejandro\\Desktop\\heterogeneous-data\\splits\\tcia_tcga_trainsplit{i}.txt", "w") as f:
        for j in train_idx[i]:
            f.write(X[j]+"\n")

    with open(f"C:\\Users\\Alejandro\\Desktop\\heterogeneous-data\\splits\\tcia_tcga_testsplit{i}.txt", "w") as f:
        for j in test_idx[i]:
            f.write(X[j]+"\n")
# %%
